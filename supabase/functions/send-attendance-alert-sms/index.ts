...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!callerProfile || callerProfile.role === "student") {
      throw new Error("Only admins and teachers can send attendance alert SMS");
    }

    const body: RequestBody = await req.json();
    const { studentPhone, studentName, className, attendanceRate, presentCount, totalCount } = body;

    if (
      !studentPhone || !studentName || !className ||
      !Number.isFinite(attendanceRate) ||
      !Number.isFinite(presentCount) ||
      !Number.isFinite(totalCount)
    ) {
      throw new Error("Missing required fields for attendance alert SMS");
    }
    ...
    console.error("Error in send-attendance-alert-sms:", err);
    return new Response(
