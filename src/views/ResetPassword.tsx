...
type Step = 'verify' | 'choose' | 'newPassword' | 'tokenReset' | 'success';
...
export default function ResetPassword() {
  ...
  useEffect(() => {
    // Hash fragments are client-only — read after mount to avoid SSR mismatch
    const token = window.location.hash.slice(1).split('=')[1] ?? '';
    setAccessToken(token);
    setTokenChecked(true);
    if (token) {
      // JWT tokens have exactly 2 dots (header.payload.signature) → Settings "Send Reset Link" flow
      // Hex tokens (forgot password) have no dots → requires identity verification first
      setStep(token.split('.').length === 3 ? 'tokenReset' : 'verify');
    }
  }, []);
  ...
  // -------- Token-based reset (from Settings "Send Reset Link") --------
  const handleTokenPasswordChange = async (e: { preventDefault: () => void }) => {
    e.preventDefault();
    setError('');
    if (password.length < 8) { setError('Password must be at least 8 characters.'); return; }
    if (password !== confirmPassword) { setError('Passwords do not match.'); return; }
  ...
  const stepLabel: Record<Step, string> = {
    ...
    tokenReset: 'Set a new password for your account',
    success: '',
  };
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.4 }} className="relative w-full max-w-md z-10">
          <div className="glass-panel rounded-[2rem] shadow-2xl shadow-black/60 border border-white/10 overflow-hidden">
            ...
            <div className="px-8 py-6 space-y-4">
              ...
              <button onClick={() => router.push('/auth')} className="w-full bg-gradient-to-r from-[#fc0ce4] to-[#949ce4] text-white py-3.5 rounded-2xl font-semibold text-sm flex items-center justify-center gap-2 hover:opacity-90 transition-all shadow-[0_0_20px_rgba(252,12,228,0.2)]">
                Go to Login
        ...
        >
          <div className="glass-panel rounded-[2rem] shadow-2xl shadow-black/60 border border-white/10 overflow-hidden">
            ...
            <div className="px-8 py-6">
              ...
              {/* ── TOKEN RESET (from Settings "Send Reset Link") ── */}
              {step === 'tokenReset' && (
                <form className="space-y-4" onSubmit={handleTokenPasswordChange}>
                  {error && (
                    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                      className="p-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-medium text-center"
                    >
                  ...
                  <p className="text-sm text-white/50">Enter a new password for your account.</p>
                  <div className="space-y-2">
                    <label className="text-[10px] lg:text-[11px] font-semibold text-white/60 uppercase tracking-widest ml-1">New Password</label>
                    <div className="relative group">
                      <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      ...
                      <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
                        placeholder="Minimum 8 characters" className="glass-input w-full pl-11 pr-4 py-3 lg:py-3.5 rounded-2xl text-sm text-white placeholder:text-white/20 bg-white/5" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] lg:text-[11px] font-semibold text-white/60 uppercase tracking-widest ml-1">Confirm Password</label>
