...
import {
  ...
  Users, Trash2, Loader2, Check, GraduationCap, AlertCircle, CheckCircle2, XCircle, Minus,
} from 'lucide-react';
...
const CLASS_COLOR = '#10b981'; // teal for class events
const DAY_NUM_H        = 34;
const SLOT_H           = 22;
...
function ParticipantBadge({ name, email }: { key?: React.Key; name?: string; email: string }) {
    ...
    <div className="h-full flex flex-col p-4 lg:p-6 gap-4 overflow-hidden">
      ...
      <div className="flex items-center justify-between shrink-0">
        <div>
          ...
          <p className="text-white/40 text-sm mt-0.5">{t('cal.teacher_desc')}</p>
        </div>
        <button
      ...
      <div className="flex items-center gap-4 shrink-0">
        ...
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded-full bg-[#fc0ce4]/60" />
          <span className="text-xs text-white/40">{t('cal.your_events')}</span>
        </div>
      </div>
              ...
              {weeks.map((week, wi) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Event pills (positioned absolutely over the grid row) */}
          ...
          <ModalShell onClose={() => setShowDetail(false)}>
            ...
            <div className="px-6 pb-5 space-y-4">
              ...
              {(() => {
                ...
                const label = selEvent.event_type === 'class' ? t('cal.students_in_class') : t('cal.participants');
                return (
                  <div>
              ...
              {/* Delete — only for own personal events */}
