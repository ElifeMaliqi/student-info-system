...
const DAY_NUM_H        = 34;  // px — height reserved for day number row
const SLOT_H           = 22;  // px — height of each event row
const MAX_VISIBLE_SLOTS = 3;  // event rows shown before '+N more'
...
interface EventLayout {
  event: CalendarEvent;
  startCol: number;       // 0-6 within this 7-day row
  span: number;           // columns occupied
  slot: number;           // vertical slot index (0 = first row under day number)
...
function layoutWeek(weekDays: Date[], events: CalendarEvent[]): EventLayout[] {
  ...
  // Longer events first for better visual ordering
  visible.sort((a, b) => {
  ...
  // occupied[col][slot] = true when taken
...
export default function AdminCalendar() {
  ...
  async function addParticipant() {
    ...
    try {
      ...
      setEmailError(t('cal.lookup_failed'));
    } finally {
      setLookingUp(false);
    }
  }
    ...
    <div className="h-full flex flex-col p-4 lg:p-6 gap-4 overflow-hidden">
      ...
      <div className="flex-1 bg-white/[0.03] border border-white/5 rounded-2xl flex flex-col overflow-hidden min-h-0">
        ...
        <div className="flex items-center gap-2 px-5 py-3 border-b border-white/5 shrink-0">
          <button
            onClick={prev}
            className="w-8 h-8 rounded-lg grid place-items-center text-white/40 hover:text-white hover:bg-white/10 transition-all"
          >
          ...
          {!isCurrentMonth && (
            <button
              onClick={goToday}
              className="ml-1 px-3 py-1 rounded-lg border border-white/10 text-white/40 hover:text-white hover:border-white/30 text-xs font-medium transition-all"
            >
              {t('cal.today')}
            </button>
          )}
        ...
        <div className="grid grid-cols-7 border-b border-white/5 shrink-0">
          {DAYS.map((d, i) => (
            <div
              key={d}
              className={`py-2 text-center text-[11px] font-semibold tracking-widest uppercase ${
                i === 0 || i === 6 ? 'text-white/20' : 'text-white/25'
              }`}
            >
              {d}
            </div>
          ...
          {loading ? (
            ...
            weeks.map((week, wi) => {
              ...
              return (
                <div
                  key={wi}
                  className={`relative ${wi < 5 ? 'border-b border-white/5' : ''}`}
                  style={{ height: rowH }}
                >
                  {/* Clickable day-cell backgrounds — absolutely fill the row */}
                    ...
                    {week.map((day, di) => (
                      <div
                        ...
                        className={`cursor-pointer transition-colors hover:bg-white/[0.025] ${
                          di < 6 ? 'border-r border-white/5' : ''
                        } ${di === 0 || di === 6 ? 'bg-white/[0.012]' : ''}`}
                      />
                    ))}
                  </div>

                  {/* Day numbers (pointer-events-none so clicks reach the cell below) */}
                  <div
                  ...
                  >
                    {week.map((day, di) => {
                      ...
                      return (
                        <div key={di} className="px-2 pt-2 flex justify-end">
                          <span className={`w-6 h-6 flex items-center justify-center rounded-full text-xs font-medium transition-all ${
                            isToday
                              ? 'bg-[#fc0ce4] text-white font-bold shadow-[0_0_12px_rgba(252,12,228,0.5)]'
                              : isCurMonth
                                ? 'text-white/55'
                                : 'text-white/18'
                          }`}>
            ...
            <div className="px-6 py-4 space-y-4 max-h-[65vh] overflow-y-auto custom-scrollbar">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={t('cal.desc_placeholder')}
                  rows={2}
                ...
                <div className="flex gap-1 p-1 rounded-xl bg-white/5 border border-white/10 mb-3">
                  {(['everyone', 'all_admins', 'all_teachers', 'custom'] as const).map(mode => (
                    <button
                      key={mode}
                      type="button"
                      onClick={() => void changeAudience(mode)}
                      disabled={loadingAudience}
                      className={`flex-1 px-1.5 py-1.5 rounded-lg text-[11px] font-medium transition-all ${
                        audienceMode === mode
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input
                        ...
                        placeholder={t('cal.email_placeholder')}
          ...
          <ModalShell onClose={() => setShowDetail(false)}>
            ...
            <div className="px-6 pb-5 space-y-4">
              {selEvent.description && (
                <p className="text-sm text-white/55 leading-relaxed">{selEvent.description}</p>
              )}
                ...
                return (
                  <div>
                    <p className="text-[11px] font-semibold text-white/25 uppercase tracking-widest flex items-center gap-1.5 mb-2">
                      <Users className="w-3 h-3" />
                      {t('cal.participants')} ({others.length})
                    </p>
